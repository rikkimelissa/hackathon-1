...
def node1cb(data, extra_args):
    ...
    if len(cnts) > 0:
        ...
        if radius > 20:
            ...
            cv2.circle(frame1, (int(x), int(y)), int(radius),(0, 255, 0), 5 )
...
def node1():
    ...
    pubx = rospy.Publisher('node1pub', UInt16, queue_size=10)
    puby = rospy.Publisher('node1pub2', UInt16, queue_size=10)
    
    rospy.Subscriber('usb_cam/image_raw', Image, node1cb, callback_args=(bridge, pubx, puby))
